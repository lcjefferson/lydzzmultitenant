...
function pickField(obj: unknown, ...keys: string[]): unknown {
  ...
  const byLower = new Map(Object.keys(record).map((k) => [k.toLowerCase(), k]));
  for (const key of keys) {
...
export class UazapiService {
  ...
  private async requestWithRetry<T>(
    ...
    try {
      ...
      if (retries > 0 && err.response && err.response.status === 429) {
        this.logger.warn(
          `Rate limit exceeded for ${url}. Retrying in ${delay}ms... (${retries} retries left)`,
        );
  ...
  async sendMessage(
    ...
    try {
      ...
      const cleanNumber =
        channelType === 'whatsapp' ? to.replace(/\D/g, '') : to;
      ...
      this.logger.log(
        `Sending Uazapi message to ${url} with payload: ${JSON.stringify(payload)}`,
      );

      const response = await this.requestWithRetry('post', url, payload, {
        headers: {
      ...
      this.logger.log(
        `Message sent to ${to} via Uazapi. Status: ${response.status}`,
      );
      return true;
    } catch (error) {
      const err = error;
      ...
      if (err.response) {
        this.logger.error(
          `Uazapi error response data: ${JSON.stringify(err.response.data)}`,
        );
        this.logger.error(
          `Uazapi error response status: ${err.response.status}`,
        );
        this.logger.error(
          `Uazapi error response headers: ${JSON.stringify(err.response.headers)}`,
        );
      }
      return false;
    }
  ...
  async sendMenu(
    ...
    try {
      ...
      if (payload.selectableCount != null)
        body.selectableCount = payload.selectableCount;
      if (payload.delay != null)
        body.delay = Math.min(5000, Math.max(0, payload.delay));

      this.logger.log(
        `Sending Uazapi menu (${payload.type}) to ${cleanNumber}`,
      );
      ...
      this.logger.log(
        `Menu sent to ${to} via Uazapi. Status: ${response?.status}`,
      );
      return true;
    } catch (error) {
      const err = error;
      this.logger.error(
        `Failed to send Uazapi menu: ${err.message ?? 'unknown'}`,
      );
      if (err.response) {
        this.logger.error(
          `Uazapi menu error: ${JSON.stringify(err.response.data)}`,
        );
      }
      return false;
    }
  ...
  async sendMediaMessage(
    ...
    try {
      ...
      const isAudio =
        mediaType === 'audio' ||
        (fileName && fileName.toLowerCase().endsWith('.webm'));
      ...
      if (appUrl && mediaUrl.startsWith(appUrl)) {
        try {
          ...
          if (fs.existsSync(absolutePath)) {
            ...
            this.logger.warn(
              `Local file not found for Base64 conversion: ${absolutePath}`,
            );
          }
        } catch (error) {
          this.logger.warn(
            `Failed to convert local file to Base64: ${error.message}`,
          );
        }
      }
      ...
      this.logger.log(
        `Sending Uazapi media to ${url} with payload: ${JSON.stringify({ ...payload, file: payload.file.substring(0, 50) + '...' })}`,
      );
      ...
      this.logger.log(
        `Media sent to ${to} via Uazapi. Status: ${response.status}`,
      );
      return true;
    } catch (error) {
      const err = error;
      ...
      if (err.response) {
        this.logger.error(
          `Uazapi error response: ${JSON.stringify(err.response.data)}`,
        );
      }
      return false;
    }
  ...
  async downloadMedia(
    ...
    try {
      ...
      if (base64Data) {
        this.logger.log(
          `Download media success. Base64 length: ${base64Data.length}`,
        );
        ...
        this.logger.log(
          `Base64 missing, attempting to download from link: ${link}`,
        );
        try {
          ...
          const mimetype =
            linkResponse.headers['content-type'] ||
          ...
          this.logger.error(
            `Failed to download from link: ${(linkError as Error).message}`,
          );
        }
      }

      this.logger.error(
        `Download media response missing base64 and link. Data keys: ${Object.keys(response.data || {}).join(', ')}`,
      );
      if (response.data) {
      ...
      this.logger.error(
        `Error downloading media from Uazapi [${targetUrl}]: ${err.message}`,
      );

      // Log connection details for debugging
      if (err.code === 'ECONNREFUSED') {
        this.logger.error(
          `Connection refused! Check if UAZAPI_API_URL (${baseUrl}) is correct and reachable from inside the docker container.`,
        );
      }
  ...
  /**
   * Inicia a conexão da instância ao WhatsApp (POST /instance/connect).
   * Sem `phone` no body, a Uazapi gera um QR Code para escanear.
   */
  async connectInstance(
    ...
    try {
      ...
      const instance = (data.instance || {}) as Record<string, any>;
      return {
        success: true,
        qrcode: instance.qrcode || data.qrcode || undefined,
        paircode: instance.paircode || data.paircode || undefined,
        status: instance.status || data.status || 'connecting',
        instanceId:
          instance.id || instance.instanceId || data.instanceId || undefined,
      };
    } catch (error) {
      const err = error;
      const detail =
        err.response?.data?.message || err.response?.data?.error || err.message;
      this.logger.error(`Uazapi connect failed: ${detail}`);
      return { success: false, error: String(detail) };
    }
  }

  /**
   * Consulta o status da instância (GET /instance/status).
   * Durante a conexão, retorna também o QR Code atualizado.
   */
  async getInstanceStatus(
    ...
    try {
      ...
      const data = response.data || {};
      const instance = (data.instance || {}) as Record<string, any>;
      const status = String(instance.status || data.status || '').toLowerCase();
      const connected =
        status === 'connected' ||
        data.connected === true ||
        data.loggedIn === true;
      ...
      const detail =
        err.response?.data?.message || err.response?.data?.error || err.message;
      this.logger.error(`Uazapi status check failed: ${detail}`);
      return { success: false, error: String(detail) };
    }
  }
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        const instanceId =
          data.instanceId ||
          data.id ||
          (data.instance && data.instance.instanceId);
        return { success: true, instanceId };
      }
      return { success: false };
    } catch (error) {
      this.logger.error(
        `Uazapi connection check failed: ${(error as Error).message}`,
      );
      return { success: false };
    }
  }
  ...
  async findMessages(
    ...
    try {
      ...
      this.logger.error(
        `Error finding messages from Uazapi: ${(error as Error).message}`,
      );
      return [];
    }
  }
  ...
  parseIncomingMessage(payload: any): {
    ...
    try {
      this.logger.log(
        `Parsing payload keys: ${Object.keys(payload).join(', ')}`,
      );
      ...
      if (
        ...
        if (
          payload.owner &&
          !payload.instanceId &&
          !looksLikePhoneNumber(payload.owner)
        ) {
          payload.instanceId = payload.owner;
        }
      } else if (
        (eventType === 'messages' || eventType === 'MESSAGE') &&
        rawEvent &&
      ...
      if (typeof fortalabsText === 'string') {
        ...
        if (
          !textBody &&
          nested.extendedTextMessage &&
          typeof nested.extendedTextMessage === 'object'
        ) {
      ...
      let type =
        firstString(
          pickField(messageData, 'type', 'messageType', 'MessageType'),
        ) || 'text';
      ...
      const token =
        firstString(pickField(payload, 'token', 'Token', 'apikey', 'apiKey')) ||
        firstString(
          pickField(messageData, 'token', 'Token', 'apikey', 'apiKey'),
        );
      ...
      let instanceId =
        ...
        firstString(pickField(payload, 'instanceName', 'InstanceName')) ||
        firstString(pickField(messageData, 'instanceName', 'InstanceName'));
      ...
      if (
        !instanceId &&
        ownerCandidate &&
        !looksLikePhoneNumber(ownerCandidate)
      ) {
      ...
      const isMediaContent = (content: any) =>
        ...
        (content.url || content.URL || content.mimetype);
        ...
        if (msg.imageMessage) {
          ...
          media = {
            ...
            base64:
              ...
              (typeof msg.imageMessage.url === 'string' &&
              msg.imageMessage.url.startsWith('data:')
      ...
      this.logger.log(
        `Extracted data - From: ${from}, Msg: ${textBody}, Type: ${type}, Inst: ${instanceId}`,
      );
      ...
      if (!from || !textBody) {
        ...
        this.logger.warn(
          `Missing required fields (from or message). From: '${from}', TextBody: '${textBody}'`,
        );
        this.logger.warn(
          `Dump messageData: ${JSON.stringify(messageData, null, 2)}`,
        );
        return null;
      }
