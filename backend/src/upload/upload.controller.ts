...
export class UploadController {
  ...
  constructor() {
    this.logger.log(
      'UploadController initialized. FFmpeg Path: ' + ffmpegInstaller.path,
    );
  ...
  async uploadFile(@UploadedFile() file: Express.Multer.File) {
    ...
    this.logger.log(
      `UPLOAD_DEBUG: Received file ${file.originalname} with mimetype ${file.mimetype}`,
    );
    ...
    if (isWebm) {
      try {
        ...
        this.logger.log(
          `Attempting to convert WebM to OGG/Opus: ${inputPath} -> ${outputPath}`,
        );
        ...
        if (fs.existsSync(outputPath)) {
          ...
          this.logger.log(
            `Audio convertido para OGG: ${finalFilename} (${stats.size} bytes)`,
          );
          try {
            fs.unlinkSync(inputPath);
          } catch {
            /* remove webm original */
          }
          return {
        ...
        this.logger.error(
          `Failed to convert WebM to OGG: ${(error as Error).message}`,
        );
