...
export class WebhooksController {
  ...
  async verifyFacebookWebhook(
    ...
    const defaultToken =
      process.env.FACEBOOK_VERIFY_TOKEN || 'facebook_leads_verify_token';
    let verifyToken = defaultToken;
  ...
  async handleWhatsAppWebhook(
    ...
    try {
      if (!payload?.entry?.length) {
        this.logger.warn(
          'WhatsApp webhook POST: payload missing entry, returning 200',
        );
        return { status: 'ok' };
      }
      ...
      const resolved =
        await this.channelResolver.resolveOfficialWhatsAppChannel(
  ...
  uazapiHealth() {
    return {
      status: 'ok',
      message:
        'Uazapi webhook endpoint is ready (POST to this URL to send events)',
    };
  }
  ...
  async handleUazapiWebhook(
    ...
    try {
      ...
      if (!incomingMessage) {
        this.logger.warn(
          'Incoming Uazapi webhook ignored: no message found in parseIncomingMessage',
        );
        return { status: 'ignored' };
      }
      ...
      if (incomingMessage.fromMe) {
        this.logger.log(
          `Ignoring message sent by me (echo) from ${incomingMessage.from}`,
        );
        return { status: 'ignored_from_me' };
      }

      this.logger.log(
        `Parsed message: ${JSON.stringify(incomingMessage, null, 2)}`,
      );
      ...
      this.logger.log(
        `Processing Uazapi message from ${incomingMessage.from}, type: ${incomingMessage.type}`,
      );
      ...
      if (incomingMessage.media) {
        try {
          ...
          if (incomingMessage.media.base64) {
            const matches = incomingMessage.media.base64.match(
              /^data:([A-Za-z-+\/]+);base64,(.+)$/,
            );
          ...
          // If we have no base64 but have mediaKey/messageId, try to download
            ...
            if (!token) {
              this.logger.warn(
                `No token found in channel config for media download (message ${incomingMessage.messageId})`,
              );
            }

            this.logger.log(
              `Processing media for message ${incomingMessage.messageId}. Token available: ${!!token}`,
            );

            if (token) {
              this.logger.log(
                `Attempting to download media for message ${incomingMessage.messageId} using token ${token.substring(0, 5)}...`,
              );
              const downloaded = await this.uazapiService.downloadMedia(
              ...
              if (downloaded) {
                ...
                this.logger.log(
                  `Media downloaded successfully: ${mimetype} (${buffer.length} bytes)`,
                );
              } else {
                this.logger.warn(
                  `Failed to download media for message ${incomingMessage.messageId} using token. Will attempt fallback to URL if available.`,
                );
              }
            } else {
              this.logger.warn(
                `No token found for media download (Channel config or ENV) for message ${incomingMessage.messageId}`,
              );
            }
          }
          ...
          if (
            !buffer &&
            incomingMessage.media.url &&
            incomingMessage.media.url.startsWith('http')
          ) {
            try {
              this.logger.log(
                `Attempting to download media from URL: ${incomingMessage.media.url}`,
              );
              ...
              if (
                ...
                this.logger.warn(
                  `Downloaded content from URL is ${contentType}, likely an error page or expired link. Discarding.`,
                );
                if (response.data.length < 1000) {
                  this.logger.debug(
                    `Content preview: ${response.data.toString()}`,
                  );
                }
              } else {
                ...
                this.logger.log(
                  `Downloaded media from URL. Size: ${buffer.length}, Type: ${mimetype}`,
                );
              }
            } catch (err) {
              this.logger.error(
                `Failed to download media from URL: ${incomingMessage.media.url}. Error: ${(err as Error).message}`,
              );
              if (axios.isAxiosError(err) && err.response) {
                this.logger.error(
                  `Fallback download status: ${err.response.status}`,
                );
              }
            }
          }
          ...
          if (buffer) {
            ...
            if (mappedType === 'text') {
              ...
              this.logger.log(
                `Corrected message type from text to ${mappedType} based on attachments`,
              );
            }
          } else {
            this.logger.warn(
              `Failed to process media for message ${incomingMessage.messageId}. No valid buffer or URL available.`,
            );
      ...
      if (existingMessage) {
        this.logger.log(
          `Message ${incomingMessage.messageId} already exists. Checking for updates...`,
        );
        ...
        if (attachments && Object.keys(attachments).length > 0) {
          this.logger.log(
            `Updating existing message ${existingMessage.id} with new attachments.`,
          );
        ...
        this.logger.log(
          `Message ${incomingMessage.messageId} is a duplicate and adds no new info. Ignoring.`,
        );
